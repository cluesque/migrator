...
var dateNowString = function() {
  // Amazing this is so painful in node ... am I missing something? -- whk 20120412
  var now = new Date();
  return "" +
        now.getFullYear() +
...
module.exports.migrate = function (app_root, app_config, options) {
  ...
  if (process.argv.length == 2) {
    ...
    if (process.argv[3] == "js") {
      ...
      var cmd = "echo 'module.exports.migrate = function(db, callback) {\n}' > " + filename + "; "+editor+" " + filename +(options.background ? " &": "");
      exec(cmd);
      ...
      var cmd = "nohup "+editor+" " + filename +(options.background ? " &": "");
      exec(cmd);
      if (options.background)
        process.exit(1);
    }
